        ...
        <div className="flex items-center justify-between h-14 sm:h-16">
          ...
          <div className="hidden lg:flex items-center gap-1 flex-1 justify-center mx-4">
            {navLinks.map((link, index) => (
              <Link key={link.path} to={link.path} className="animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
                <Button variant={isActive(link.path) ? "default" : "ghost"} size="sm" className="gap-2 transition-smooth hover-lift">
          ...
          <div className="flex lg:hidden items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            <button
              ...
              aria-label="القائمة"
            >
              {isOpen ? <X className="h-5 w-5 sm:h-6 sm:w-6 transition-transform duration-200" /> : <Menu className="h-5 w-5 sm:h-6 sm:w-6 transition-transform duration-200" />}
            </button>
          </div>
        </div>
          ...
          <div className="py-3 sm:py-4 space-y-1.5 sm:space-y-2">
            {navLinks.map((link) => (
              <Link key={link.path} to={link.path} onClick={() => setIsOpen(false)} className="block">
                <Button variant={isActive(link.path) ? "default" : "ghost"} size="sm" className="w-full justify-start gap-2 transition-smooth hover-lift active:scale-[0.98]">
                  <link.icon className="h-4 w-4" />
                  {link.name}
                </Button>
              </Link>
